...
const handler = async function (event, context) {
  ...
  const strength = event.queryStringParameters.strength.trim();
  const complete = false;
  const hero = { id, name, strength, complete };
  console.log(hero);

  try {
    const response = await fetch(
      "https://data.mongodb-api.com/app/data-xajht/endpoint/data/beta/action/insertOne",
      {
        method: "POST",
        headers: {
